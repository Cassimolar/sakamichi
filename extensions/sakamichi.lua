    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.CardUsed then
            ...
            if use.card and use.card:isKindOf("Duel") and use.card:getSkillName() == self:objectName() and
                use.from:objectName() == player:objectName() and player:hasSkill(self) then
...
sakamichi_bao_zhong = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.PreHpRecover then
            local recover = data:toRecover()
            if recover.who and recover.who:isFemale() and player:hasSkill(self) and
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if move.from and move.from:objectName() ~= player:objectName() then
            if move.to_place == sgs.Player_DiscardPile then
                ...
                if not card_ids:isEmpty() and player:getMark("@da_pei") ~= 0 and
                    room:askForSkillInvoke(player, self:objectName(), data) then
...
sakamichi_hei_shi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local recover = data:toRecover()
        if recover.who and recover.who:objectName() ~= player:objectName() and recover.who:hasSkill(self) and
...
sakamichi_sheng_qi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start and player:hasSkill(self) and
            player:getMark("@sheng_qi") ~= 0 then
...
sakamichi_bu_liang = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.Death then
            local death = data:toDeath()
            if death.who:hasSkill(self) and death.damage and death.damage.from and
...
sakamichi_dan_shi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start and player:hasSkill(self) and
            player:getMark("@dan_shi") ~= 0 and room:askForSkillInvoke(player, self:objectName(), data) then
            local target = room:askForPlayerChosen(player, room:getOtherPlayers(player), self:objectName(),
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.SlashMissed then
            ...
            if effect.slash:isBlack() and effect.from and effect.from:isAlive() and effect.to and
                effect.to:hasSkill(self) and effect.to:getMark(self:objectName()) == 0 then
        ...
        elseif event == sgs.SlashHit then
            local effect = data:toSlashEffect()
            if effect.slash:hasFlag("bu_she_yuki_yoda") and effect.from and effect.from:hasSkill(self) and
        ...
        elseif event == sgs.Damaged then
            local damage = data:toDamage()
            if damage.card and damage.card:isKindOf("Slash") and damage.card:isBlack() and player:hasSkill(self) and
                player:getMark(self:objectName()) == 0 then
...
sakamichi_shen_chang = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.SlashProceed then
            local effect = data:toSlashEffect()
            if effect.from:hasSkill(self) and effect.from:distanceTo(effect.to) ==
                SKMC.number_correction(effect.from, 1) then
                room:slashResult(effect, nil)
                return true
            end
...
sakamichi_kou_sha = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if dying.damage and dying.damage.card and dying.damage.card:isKindOf("Slash") and dying.damage.from and
            dying.damage.from:hasSkill(self) and dying.damage.from:hasEquipArea() and
...
sakamichi_ti_cao = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.SlashHit then
            local effect = data:toSlashEffect()
            if effect.from and effect.from:objectName() == player:objectName() and player:hasSkill(self) and
        ...
        elseif event == sgs.SlashMissed then
            local effect = data:toSlashEffect()
            if effect.from and effect.from:objectName() == player:objectName() and effect.to:hasSkill(self) and
...
sakamichi_da_zhi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.CardsMoveOneTime then
            local move = data:toMoveOneTime()
            if move.to and move.to:objectName() == player:objectName() and player:hasSkill(self) and move.to_place and
                move.to_place == sgs.Player_PlaceSpecial and move.to_pile_name and move.to_pile_name == "gong_yan" then
...
sakamichi_chuan_cheng = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.EnterDying then
            local dying = data:toDying()
            if dying.who and dying.who:objectName() == player:objectName() and player:hasSkill(self) and
...
sakamichi_sheng_ye = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.CardsMoveOneTime then
            local move = data:toMoveOneTime()
            if move.to and move.to:objectName() == player:objectName() and player:hasSkill(self) and move.to_place and
                move.to_place == sgs.Player_PlaceSpecial and move.to_pile_name and move.to_pile_name == "qi_ji" then
                room:addPlayerMark(player, "&" .. self:objectName(), move.card_ids:length())
            end
        elseif event == sgs.EventPhaseProceeding then
            if player:hasSkill(self) and player:getPhase() == sgs.Player_Start and player:getMark(self:objectName()) ==
                0 and player:getMark("&" .. self:objectName()) >= 20 then
        ...
        elseif event == sgs.Death then
            local death = data:toDeath()
            if death.who:objectName() == player:objectName() and player:hasSkill(self) and
                player:getMark(self:objectName()) == 0 then
...
sakamichi_xie_zui = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.Damage or event == sgs.Damaged then
            local damage = data:toDamage()
            if (event == sgs.Damage and damage.from:objectName() == player:objectName() and player:hasSkill(self)) or
                (event == sgs.Damaged and damage.to:objectName() == player:objectName() and player:hasSkill(self)) then
        ...
        elseif event == sgs.Death then
            local death = data:toDeath()
            if death.who:objectName() == player:objectName() and player:hasSkill(self) and death.damage and
...
sakamichi_guan_xing = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.EventPhaseProceeding then
            if player:getPhase() == sgs.Player_Start and player:hasSkill(self) and player:getMark(self:objectName()) ~=
                0 and player:getMark(self:objectName() .. "_can_trigger") ~= 0 then
...
sakamichi_qi_shi_hidden_effect = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local dying = data:toDying()
        if dying.who:objectName() == player:objectName() and player:isLord() and
            (string.find(player:getGeneralName(), "ArunoNakanishi") or
                string.find(player:getGeneral2Name(), "ArunoNakanishi")) then
            local change = false
            for _, p in sgs.qlist(room:getOtherPlayers(player)) do
                if string.find(p:getGeneralName(), "AsukaSaito") or string.find(p:getGeneral2Name(), "AsukaSaito") or
                    string.find(p:getGeneralName(), "MizukiYamashita") or
                    string.find(p:getGeneral2Name(), "MizukiYamashita") then
                    change = true
...
sakamichi_si_mo_card = sgs.CreateSkillCard {
    ...
    filter = function(self, targets, to_select)
        return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and
                   to_select:hasSkill(self:getSkillName())
    end,
    on_effect = function(self, effect)
        local room = effect.from:getRoom()
        local choices = {}
...
sakamichi_si_mo = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if (event == sgs.GameStart and player:hasSkill(self)) or
        ...
        if move.from and move.to and move.to:objectName() ~= move.from:objectName() and move.from:objectName() ==
            ...
            if n ~= 0 then
                local card = room:askForExchange(room:findPlayerByObjectName(move.to:objectName()), self:objectName(),
                                                 n, n, false,
                                                 "@yuan_wei_invoke:" .. move.from:objectName() .. "::" .. n, true)
                if card then
                    room:obtainCard(player, card, false)
                else
                    room:damage(sgs.DamageStruct(self:objectName(), player,
...
sakamichi_xie_lou = sgs.CreateTriggerSkill {
    ...
    events = {sgs.EventPhaseProceeding},
    on_trigger = function(self, event, player, data, room)
        if not player:isKongcheng() and player:getPhase() == sgs.Player_Start and player:getKingdom() == "Nogizaka46" and
            SKMC.is_ki_be(player, 5) then
            for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
...
sakamichi_bo_xue = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        elseif event == sgs.Damaged then
            local damage = data:toDamage()
            if player:hasSkill(self) and damage.card and damage.card:hasFlag("bo_xue") and
...
sakamichi_wu_mei = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.TargetConfirming then
            local use = data:toCardUse()
            if not use.card:isKindOf("SkillCard") and use.from and use.from:hasSkill(self) and use.to:contains(player) and
...
sakamichi_bu_ya = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if death.who:objectName() == player:objectName() then
            for _, p in sgs.qlist(room:getOtherPlayers(player)) do
                if (player:getMark("shi_sheng_" .. p:objectName() .. player:objectName()) ~= 0 and player:hasSkill(self)) or
                    (p:getMark("shi_sheng_" .. player:objectName() .. p:objectName()) ~= 0 and p:hasSkill(self)) then
                    p:throwAllHandCardsAndEquips()
                    p:turnOver()
                end
            end
...
sakamichi_li_fa = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local use = data:toCardUse()
        if not use.card:isKindOf("SkillCard") and use.from and use.from:objectName() ~= player:objectName() and
            use.from:getHandcardNum() >= player:getHandcardNum() and
            room:askForSkillInvoke(player, self:objectName(), data) then
            if not room:askForDiscard(use.from, self:objectName(), 1, 1, true, true,
                                      "@li_fa_discard:" .. player:objectName() .. "::" .. use.card:objectName(), ".",
                                      self:objectName()) then
                local nullified_list = use.nullified_list
                table.insert(nullified_list, player:objectName())
                use.nullified_list = nullified_list
...
sakamichi_hu_la = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local move = data:toMoveOneTime()
        if move.from and move.from:objectName() == player:objectName() and player:getPhase() == sgs.Player_NotActive and
            move.from_places and move.from_places:contains(sgs.Player_PlaceSpecial) and
            table.contains(move.from_pile_names, "&hu_la") then
...
sakamichi_guan_aiCard = sgs.CreateSkillCard {
    ...
    filter = function(self, targets, to_select)
        return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and
                   not sgs.Self:hasFlag("guan_ai_" .. to_select:objectName())
    end,
    on_effect = function(self, effect)
        local room = effect.from:getRoom()
        ...
        local card = room:askForCard(effect.from, ".|.|.|hand",
                                     "@guan_ai:" .. effect.to:objectName() .. "::" .. suit_str, sgs.QVariant(),
                                     sgs.Card_MethodNone, nil, false, self:getSkillName(), false)
        if card then
            room:showCard(effect.from, card:getEffectiveId())
        ...
        elseif event == sgs.BeforeCardsMove or event == sgs.CardsMoveOneTime then
            ...
            if move.from and move.from:objectName() == player:objectName() and
                ...
                if event == sgs.BeforeCardsMove then
                    ...
                    if move.from:getMaxCards() == 0 and move.from:getPhase() == sgs.Player_Discard and
                        bit32.band(move.reason.m_reason, sgs.CardMoveReason_S_MASK_BASIC_REASON) ==
                        sgs.CardMoveReason_S_REASON_RULEDISCARD then
                            ...
                            if p:getMark(self:objectName()) ~= 0 and room:askForSkillInvoke(p, self:objectName(), data) then
                                ...
                                if p:objectName() ~= player:objectName() then
                                    local card = room:askForCard(p, ".|.|.|hand",
                                                                 "@hui_zhang:" .. move.from:objectName(), data,
                                                                 sgs.Card_MethodNone)
                                    if card then
                ...
                for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                    if not p:getPile("xin_li_xue"):isEmpty() then
                        ...
                        local suit, number, name = card1:getSuit() == card2:getSuit(),
                                                   ...
                                                   SKMC.true_name(card1) == SKMC.true_name(card2)
            ...
            for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                if not p:getPile("xin_li_xue"):isEmpty() then
                    room:throwCard(sgs.Sanguosha:getCard(p:getPile("xin_li_xue"):first()), sgs.CardMoveReason(
                                       sgs.CardMoveReason_S_REASON_REMOVE_FROM_PILE, "", p:objectName(),
                                       self:objectName(), ""), nil)
...
sakamichi_bi_ji = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local use = data:toCardUse()
        if use.card and (use.card:isKindOf("TrickCard") and not use.card:isKindOf("DelayedTrick")) and
            use.from:objectName() == player:objectName() then
            local ids = sgs.IntList()
            if use.card:isVirtualCard() then
                ids = use.card:getSubcards()
                        ...
                        if player:getHandcardNum() < player:getMaxCards() then
                            ...
                            if card then
                                ...
                                room:drawCards(player, math.min(player:getMaxCards() - player:getHandcardNum(),
                                                                SKMC.number_correction(p, 5) - player:getHandcardNum()),
                                               self:objectName())
                            end
                        else
                            break
...
sakamichi_dian_chao = sgs.CreateTriggerSkill {
    ...
    events = {sgs.EventPhaseStart},
    on_trigger = function(self, event, player, data, room)
        if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start and
            room:askForSkillInvoke(player, self:objectName(), data) then
...
sakamichi_leng_yan = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.BeforeCardsMove then
            local move = data:toMoveOneTime()
            if move.from and move.from:getPhase() == sgs.Player_NotActive and move.from:hasSkill(self) and
                move.from_places:contains(sgs.Player_PlaceHand) then
                for _, id in sgs.qlist(move.card_ids) do
                    if room:getCardPlace(id) == sgs.Player_PlaceHand then
        ...
        elseif player:getPhase() == sgs.Player_Finish then
            for _, p in sgs.qlist(room:getAlivePlayers()) do
                if p:getMark("leng_yan_num") ~= 0 then
                    if p:getMark("leng_yan_num") >= 2 and
                        room:askForSkillInvoke(p, self:objectName(), sgs.QVariant("damage:" .. player:objectName())) then
                        room:damage(sgs.DamageStruct(self:objectName(), p, player, SKMC.number_correction(p, 1)))
                    end
...
sakamichi_man_re = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if player:getPhase() == sgs.Player_Play then
            for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                if p:objectName() ~= player:objectName() and
                    room:askForSkillInvoke(p, self:objectName(), sgs.QVariant("slash:" .. player:objectName())) then
...
sakamichi_hua_she_view_as = sgs.CreateOneCardViewAsSkill {
    name = "sakamichi_hua_she",
    filter_pattern = ".|.|.|hand",
    guhuo_type = "lsr",
    view_as = function(self, card)
        local cd = sgs.Self:getTag(self:objectName()):toCard()
        sgs.Self:removeTag(self:objectName())
        cd:addSubcard(card:getId())
        cd:setSkillName(self:objectName())
        return cd
    end,
...
sakamichi_hua_she = sgs.CreateTriggerSkill {
    name = "sakamichi_hua_she",
    view_as_skill = sakamichi_hua_she_view_as,
    events = {sgs.CardUsed, sgs.CardResponded},
    on_trigger = function(self, event, player, data, room)
        local card
        if event == sgs.PreCardUsed then
            ...
            if use.card:isKindOf("Collateral") and use.card:hasFlag("chong_fu_2") then
                for _, p in sgs.qlist(use.to) do
                    ...
                    if pl_list:isEmpty() then
                        ...
                        local victim = room:askForPlayerChosen(player, pl_list, self:objectName(),
                                                               "@chong_fu_collateral:" .. p:objectName() .. "::" ..
                                                                   use.card:objectName())
                        local _data = sgs.QVariant()
                        _data:setValue(victim)
                        p:setTag("collateralVictim", _data)
        ...
        if not use.card:isKindOf("SkillCard") then
            for _, p in sgs.qlist(use.to) do
                local card = room:askForCard(player, ".|.|.|hand",
                                             "@wei_xiao_give:" .. p:objectName() .. "::" .. use.card:objectName(), data,
                                             sgs.Card_MethodNone, p, false)
                if card then
...
sakamichi_zhu_luan = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if not room:getTag("FirstRound"):toBool() and move.card_ids:length() >= 2 and move.to and move.to:objectName() ==
            ...
            local current = room:getCurrent()
            if current:objectName() == player:objectName() or
                room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("draw:::" .. self:objectName())) then
...
sakamichi_di_yin = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local damage = data:toDamage()
        if damage.card and damage.card:isKindOf("Slash") and not damage.to:faceUp() and
            room:askForSkillInvoke(player, self:objectName(), data) then
            damage.damage = damage.damage + SKMC.number_correction(player, 1)
            data:setValue(damage)
        end
...
sakamichi_hui_shi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if use.card:isKindOf("Peach") and not use.card:isVirtualCard() then
            ...
            for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                if p:objectName() ~= player:objectName() and
                    p:getTag(self:objectName()):toIntList():contains(use.card:getEffectiveId()) then
                    player:turnOver()
                end
            end
        end
...
sakamichi_xin_lai = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.EventPhaseProceeding then
            if player:getPhase() == sgs.Player_Start then
                for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                    if player:objectName() ~= p:objectName() and not p:isKongcheng() and
                        room:askForSkillInvoke(p, self:objectName(), sgs.QVariant(
                        ...
                        room:moveCardTo(dummy, p, player, sgs.Player_PlaceHand, sgs.CardMoveReason(
                                            sgs.CardMoveReason_S_REASON_GIVE, p:objectName(), player:objectName(),
                                            self:objectName(), nil))
                    end
                end
            end
        ...
        if event == sgs.CardsMoveOneTime then
            ...
            if move.from and move.to and move.from:objectName() ~= move.to:objectName() and move.to:objectName() ==
                ...
                if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("invoke:" .. move.from:objectName())) then
                    room:damage(sgs.DamageStruct(self:objectName(), player,
                                                 room:findPlayerByObjectName(move.from:objectName()),
                                                 SKMC.number_correction(player, 1)))
                end
            end
        else
...
sakamichi_tian_ka = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.Damage then
            local damage = data:toDamage()
            if damage.to:hasSkill(self) and player:getPhase() ~= sgs.Player_NotActive and
                room:askForSkillInvoke(damage.to, self:objectName(), data) then
...
LuayouyanCard = sgs.CreateSkillCard {
    ...
    filter = function(self, targets, to_select)
        return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and
                   not sgs.Self:hasFlag("youyan_used" .. to_select:objectName())
    end,
    on_effect = function(self, effect)
        local room = effect.from:getRoom()
        local type1
...
sakamichi_tou_dai_view_as = sgs.CreateOneCardViewAsSkill {
    ...
    view_filter = function(self, to_select)
        return (not to_select:isKindOf("BasicCard") and to_select:getSuit() == sgs.Card_Heart) or
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if effect.from and effect.from:objectName() == player:objectName() and player:getWeapon() then
            ...
            if effect.to:getArmor() then
                if room:askForSkillInvoke(player, self:objectName(), sgs.QVariant(
                                              "armor:" .. effect.to:objectName() .. "::" ..
                                                  effect.to:getArmor():objectName())) then
...
sakamichi_ren_nai = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.Damaged then
            local damage = data:toDamage()
            if damage.nature == sgs.DamageStruct_Fire and
                room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("draw:::" .. damage.damage)) then
                room:drawCards(player, damage.damage, self:objectName())
            end
        else
            local damage = data:toDamage()
            if damage.nature == sgs.DamageStruct_Normal and
                room:askForSkillInvoke(player, self:objectName(), sgs.QVariant("fire")) then
...
sakamichi_ying_dun = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.TargetConfirming then
            local use = data:toCardUse()
            if (use.card:isKindOf("Slash") or use.card:isNDTrick()) and use.to:length() == 1 and use.to:contains(player) and
                use.from:objectName() ~= player:objectName() then
...
sakamichi_ying_dun_protect = sgs.CreateProhibitSkill {
    ...
    is_prohibited = function(self, from, to, card)
        return
            to:hasSkill("sakamichi_ying_dun") and from:objectName() ~= to:objectName() and to:getMark("ying_dun_on") ~=
    ...
    on_trigger = function(self, event, player, data, room)
        if not player:isKongcheng() then
            local target = room:askForYiji(player, player:handCards(), self:objectName(), false, false, true, 1,
                                           room:getOtherPlayers(player), sgs.CardMoveReason(
                                               sgs.CardMoveReason_S_REASON_GIVE, player:objectName(), self:objectName(),
                                               nil), "@chuan_xi_invoke:::" .. self:objectName(), false)
            if target then
                local n = SKMC.number_correction(player, 1)
                if room:askForChoice(player, self:objectName(), "chuan_xi_1=" .. target:objectName() .. "=" .. n ..
                                         "+chuan_xi_2=" .. target:objectName() .. "=" .. n) == "chuan_xi_1=" ..
                    target:objectName() .. "=" .. n then
...
sakamichi_ji_ji = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if not player:isKongcheng() then
            ...
            if not targets:isEmpty() then
                local target = room:askForYiji(player, player:handCards(), self:objectName(), false, false, true, 1,
                                               targets, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_GIVE,
                                                                           player:objectName(), self:objectName(), nil),
                                               "@ji_ji_invoke", false)
                if target then
            ...
            for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
                if p:objectName() ~= player:objectName() and room:askForSkillInvoke(p, self:objectName(), data) then
                    ...
                    room:moveCardTo(sgs.Sanguosha:getCard(id), nil, nil, sgs.Player_PlaceTable, sgs.CardMoveReason(
                                        sgs.CardMoveReason_S_REASON_TURNOVER, p:objectName(), self:objectName(), nil),
                                    true)
                    room:moveCardTo(sgs.Sanguosha:getCard(id), nil, nil, sgs.Player_DrawPile, sgs.CardMoveReason(
                                        sgs.CardMoveReason_S_REASON_PUT, p:objectName(), nil, self:objectName(), nil),
                                    false)
...
sakamichi_meng_qiaoCard = sgs.CreateSkillCard {
    ...
    filter = function(self, targets, to_select)
        return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and
                   not sgs.Self:hasFlag("meng_qiao_used_" .. to_select:objectName())
    end,
    on_effect = function(self, effect)
        local room = effect.from:getRoom()
            ...
            if id ~= -1 then
                ...
                if room:askForChoice(effect.from, self:getSkillName(), table.concat(move_choices, "+")) == move_choice_1 then
                    room:moveCardTo(card, effect.to, nil, sgs.Player_DrawPile, sgs.CardMoveReason(
                                        sgs.CardMoveReason_S_REASON_PUT, effect.from:objectName(), nil,
                                        self:getSkillName(), nil), true)
                else
                    room:throwCard(card, sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_DISMANTLE,
                                                            effect.from:objectName(), nil, self:getSkillName(), nil),
...
sakamichi_pin_weiCard = sgs.CreateSkillCard {
    ...
    on_effect = function(self, effect)
        ...
        if not (effect.to:hasWeaponArea() and effect.to:hasArmorArea() and effect.to:hasDefensiveHorseArea() and
            effect.to:hasOffensiveHorseArea() and effect.to:hasTreasureArea()) then
...
sakamichi_pin_wei = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local damage = data:toDamage()
        if damage.from and damage.from:getEquips():length() < player:getEquips():length() and
            room:askForSkillInvoke(player, self:objectName(), data) then
            room:drawCards(damage.from, 1, self:objectName())
            return true
        end
...
sakamichi_zhen_yan = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local move = data:toMoveOneTime()
        if move.to and move.to:objectName() == player:objectName() and player:getPhase() == sgs.Player_NotActive and
            room:askForSkillInvoke(player, self:objectName(), data) then
            room:askForUseCard(player, "slash", "@askforslash")
        end
        return false
...
sakamichi_ling_di = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start and not player:isKongcheng() and
            room:askForSkillInvoke(player, self:objectName(), data) then
            player:throwAllHandCards()
        ...
        if card:getSuit() == sgs.Card_Heart then
            ...
            if card_ex then
                ...
                room:moveCardTo(card, nil, nil, sgs.Player_DrawPile,
                                sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_PUT, source:objectName(), nil,
                                                   self:objectName(), nil), false)
            end
...
sakamichi_yi_xiao = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        ...
        if who and who:objectName() ~= player:objectName() and card and not card:isKindOf("SkillCard") and
            player:getHandcardNum() <= player:getMaxHp() and room:askForSkillInvoke(player, self:objectName(), data) then
            room:drawCards(player, 1, self:objectName())
        end
        return false
    end,
...
sakamichi_zhu_dao = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.TurnOver then
            ...
            local dying = data:toDying()
            if dying.who:objectName() == player:objectName() and dying.damage and dying.damage.from and
                dying.damage.from:hasSkill(self) and dying.damage.reason == self:objectName() then
...
sakamichi_jiao_meng = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        if event == sgs.TargetConfirmed then
            local use = data:toCardUse()
            if player:hasSkill(self) and use.card:isKindOf("Slash") and use.from:objectName() ~= player:objectName() and
        ...
        elseif event == sgs.TargetSpecified then
            local use = data:toCardUse()
            if player:hasSkill(self) and use.card:isKindOf("Slash") and not use.to:contains(player) and
                room:askForSkillInvoke(player, self:objectName(), data) then
                for _, p in sgs.qlist(use.to) do
                    room:loseHp(player, SKMC.number_correction(player, 1))
...
sakamichi_xi_qi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local current = room:getCurrent()
        if current:objectName() ~= player:objectName() and current:getHandcardNum() >= player:getHandcardNum() and
            room:askForSkillInvoke(player, self:objectName(), data) then
...
sakamichi_tie_bi_trigger = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local move = data:toMoveOneTime()
        if move.from and move.from:objectName() == player:objectName() and player:hasSkill("sakamichi_tie_bi") and
            move.from_places:contains(sgs.Player_PlaceEquip) then
            local i = 0
            for _, card_id in sgs.qlist(move.card_ids) do
                if player:isAlive() and move.from_places:at(i) == sgs.Player_PlaceEquip and
                    sgs.Sanguosha:getCard(card_id):isKindOf("Armor") then
                    for _, mark in sgs.list(player:getMarkNames()) do
...
sakamichi_jie_su = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        for _, p in sgs.qlist(room:findPlayersBySkillName(self:objectName())) do
            if p:objectName() ~= player:objectName() and p:faceUp() and
                room:askForSkillInvoke(p, self:objectName(), sgs.QVariant("invoke:" .. player:objectName())) then
                p:turnOver()
...
sakamichi_e_yi = sgs.CreateTriggerSkill {
    ...
    on_trigger = function(self, event, player, data, room)
        local damage = data:toDamage()
        if damage.card and damage.card:isKindOf("Slash") and not damage.to:isKongcheng() and
            room:askForSkillInvoke(player, self:objectName(), data) then
