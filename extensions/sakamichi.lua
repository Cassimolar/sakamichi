...
function SKMC.fake_move(room, player, pile_name, id, movein, skill_name)
...
sgs.LoadTranslationTable {
    ...
    [":sakamichi_jing_ye"] = "限定技，准备阶段，你可以选择一名其他角色，你与其展示身份牌，若你们的胜利条件：相同，本局游戏剩余时间内，你们的手牌互相可以见；不同，本局游戏剩余时间内，一方受到伤害后另一方失去等量的体力值。",
    ["@jingye"] = "敬业",
            ...
            if not hasyurina and player:isLord() and player:getMark("@qiao_zhong") ~= 0 and room:askForSkillInvoke(player, self:objectName(), data) then
                ...
                for _, p in sgs.qlist(room:getAlivePlayers()) do
                    if p:getKingdom() == "Keyakizaka46" then
                        ...
                        if name_1 and string.find(name_1, "Keyakizaka") then
                            name_1 = string.gsub(name_1, "Keyakizaka", "Sakurazaka")
                        end
                        if name_2 and string.find(name_2, "Keyakizaka") then
                            name_2 = string.gsub(name_2, "Keyakizaka", "Sakurazaka")
                        end
                        local general_1, general_2 = sgs.Sanguosha:getGeneral(name_1), sgs.Sanguosha:getGeneral(name_2)
                        if general_1 then
                            room:changeHero(p, general_1:objectName(), false)
                        end
                        if general_2 then
                            room:changeHero(p, general_2:objectName(), false, true, true)
                        end
                    end
                end
            end
