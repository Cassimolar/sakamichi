...
struct ManualSkill
{
    ...
    {
        static const QString prefixes[] = { "boss", "gd", "jg", "jsp", "kof", "neo", "nos", "ol", "sp", "tw", "vs", "yt", "diy", "new", "tenyear",
                                            "mobile", "second", "third" };
...
void Engine::addSkills(const QList<const Skill *> &all_skills)
{
    foreach (const Skill *skill, all_skills) {
        ...
        else if (skill->inherits("ProhibitPindianSkill"))
            prohibitpindian_skills << qobject_cast<const ProhibitPindianSkill *>(skill);
