...
void ZhibaCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    ...
    PindianStruct *pindian = source->PinDian(sunce, "zhiba_pindian", NULL);
    if (!pindian) return;
    if (pindian->from_number > pindian->to_number)
        room->broadcastSkillInvoke("zhiba", 3);
    else {
        room->broadcastSkillInvoke("zhiba", 2);
        if (pindian->to && !pindian->to->isDead() && pindian->to->hasLordSkill("zhiba")) {
            ...
            if (room->getCardPlace(to_card_id) == Player::DiscardPile &&
