...
void FenweiCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    ...
    bool ganning = false;
    bool heqi = false;
    if (source->getGeneralName() == "ganning" || (source->getGeneral2() && source->getGeneral2Name() == "ganning"))
        ganning = true;
    if (source->getGeneralName() == "heqi" || (source->getGeneral2() && source->getGeneral2Name() == "heqi"))
        heqi = true;
    if (heqi && !ganning)
        room->doSuperLightbox("heqi", "fenwei");
    else
