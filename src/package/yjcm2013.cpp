...
void FenchengCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    ...
    if (source->getGeneralName() == "mobile_liru" || (source->getGeneralName() != "liru" && source->getGeneral2() && source->getGeneral2Name() == "mobile_liru"))
        room->doSuperLightbox("mobile_liru", "fencheng");
    else
...
public:
    ...
    bool trigger(TriggerEvent event, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (event == EventPhaseEnd) {
            ...
            int n = player->tag["oljingce_type"].toList().length();
            if (n <= 0) return false;
            ...
            player->tag.remove("oljingce_type");
            room->setPlayerProperty(player, "oljingce_suits", QStringList());
        }else {
            if (player->getPhase() != Player::Play) return false;
            const Card *card = NULL;
            ...
            if (used_card != NULL) {
                QVariantList list = player->tag["oljingce_type"].toList();
                int n = used_card->getTypeId();
                if (!list.contains(QVariant(n))) {
                    list << n;
            ...
            if (card != NULL) {
                ...
                QStringList list = player->property("oljingce_suits").toStringList();
                if (!list.contains(suit)) {
                    list << suit;
                    room->setPlayerProperty(player, "oljingce_suits", list);
