...
CardEffectStruct::CardEffectStruct()
    : card(NULL), from(NULL), to(NULL), multiple(false), nullified(false), no_respond(false), no_offset(false)
{
}
...
CardUseStruct::CardUseStruct()
    : card(NULL), from(NULL), m_isOwnerUse(true), m_addHistory(true), nullified_list(QStringList()), whocard(NULL), who(NULL),
      no_respond_list(QStringList()), no_offset_list(QStringList())
{
}
...
void RoomThread::addPlayerSkills(ServerPlayer *player, bool invoke_game_start)
{
    ...
    foreach (const TriggerSkill *skill, player->getTriggerSkills()) {
        ...
        if (invoke_game_start && skill->getTriggerEvents().contains(GameReady))
            invoke_verify = true;
    }
